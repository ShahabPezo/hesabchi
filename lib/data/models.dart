...
class ChequeStatusEntry {
  const ChequeStatusEntry({
    required this.status,
    required this.note,
  ...
  factory ChequeStatusEntry.fromJson(Map<String, dynamic> json) {
    return ChequeStatusEntry(
      status: _optionalText(json['status'], fallback: ''),
      note: _optionalText(json['note'], fallback: ''),
      changedAt: _optionalText(json['changedAt'], fallback: ''),
    );
  }
}
...
class Cheque {
  ...
  final String operationType;
  final String description;
  /// "in" = دریافتی، "out" = صادره
  final String direction;
  final String status;
  final List<String> allocatedTo;
  final List<ChequeStatusEntry> statusLog;
  ...
  static const _settledStatuses = {
    ...
    'برداشت مدیر', 'منتقل شده',
    'برگشت داده شده',
  };

  bool get isSettled => _settledStatuses.contains(status);
  bool get isActive => !isSettled;

  factory Cheque.fromJson(Map<String, dynamic> json) {
    final logRaw = json['statusLog'];
    final statusLog = logRaw == null
        ? <ChequeStatusEntry>[]
        : (logRaw as List)
            .map((e) => ChequeStatusEntry.fromJson(e as Map<String, dynamic>))
            .toList(growable: false);

    final allocatedRaw = json['allocatedTo'];
    final allocatedTo = allocatedRaw == null
        ? <String>[]
        : (allocatedRaw as List).map((e) => e.toString()).toList(growable: false);

    return Cheque(
      id: _optionalText(json['id'], fallback: ''),
      checkNumber: _optionalText(json['checkNumber'], fallback: ''),
...
class BusinessDataset {
  ...
  // پردازش می‌شود).
  static const maxKnownVersion = 4;
